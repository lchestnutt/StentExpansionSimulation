...
myDE = 'Deformed-Expander'
myString = name+str('-BE')
myB = 'Bender'
myS = 'Stent'
myV = 'Vessel'
...
def Apply_Displacement_BC_Expand(model, instance, set_name, BC_name, amp_name, step, CSYS_i, u1, u2, u3, ur1, ur2, ur3):
    a = mdb.models[model].rootAssembly
...
def Set_M3D4R_Elem_Type(model, part, set_name):
    elemType1 = mesh.ElemType(elemCode=M3D4R, elemLibrary=EXPLICIT, secondOrderAccuracy=OFF, hourglassControl=ENHANCED)
    p = mdb.models[model].parts[part]
    pickedRegions = p.sets[set_name]
    p.setElementType(regions=pickedRegions, elemTypes=(elemType1, ))
...
def Stent_Mesh_Membrane(myString, set_name):
    p = mdb.models[myString].parts['Stent']
    f = p.elements
    face1Elements = f.getSequenceFromMask(mask=('[#ffffffff:770 #ffffff ]', ), )
    p.generateMeshByOffset(region=regionToolset.Region(face1Elements=face1Elements), meshType=SHELL, distanceBetweenLayers=0.0, numLayers=1, initialOffset=0.0, shareNodes=True)
    e = p.elements
    elements = e.getSequenceFromMask(mask=('[#0:3083 #ffffffff:770 #ffffff ]', ), )
    p.Set(elements=elements, name=set_name)
...
Import_Stl_file(myString,myDE,ExpanderPath)
...
#Create Part Sets
Create_Set_All_Elements(myString, myDE, 'ALL-Elements')
...
#Access ref pointsfor the bender
...
Instance_to_Assembly(myString, myDE, myDE+str('-1'))
...
fix_axis = Datum_Axis_From_Principle(myString, myB, XAXIS)
Edge_Edge_Constraint(myString, myS+str('-1'), myB+str('-1'), move_axis, fix_axis, 180)

##Create Materials and Sections
Create_Material(myString, 'Steel', 7.5e-09, 193000.0, 0.3, ((200.0, 0.0), (410.0, 0.02), (530.0, 0.1), (600.0, 0.2), (630, 0.35), (640, 0.4)), 1)

Create_Material(myString, 'VesselWall', 1.06E-09, 0.75, 0.45, 'None', 'None')
...
Section_Assignment(myString, myDE, 'ALL-Elements', 'Surface-Section')
...
Create_Dynamic_Displacement_Step(myString, 'Expand', 'Bend', 2, ON)
...
bend_amp_data = ((0.0, 0.0), (0.2, 1.0))
Create_Smooth_Step_Amplitude(myString, 'Smooth-Bend', bend_amp_data)

check_2 = (0.45*diameter[0][0]-r_expander)/expander_dispSVC
expand_amp_data = ((0.,0.), (1.2,check_2), (2,1.))

Create_Smooth_Step_Amplitude(myString, 'Smooth-Expand', expand_amp_data)
...
BenderRotations.append([UNSET,UNSET,UNSET])

for d, rot, n in zip(BenderDisplacements, BenderRotations, range(0,len(BenderPoints))): 
    Create_Ref_Point(myString, BenderPoints[n])
    Create_Set_Node_Ring_Cyl(myString, myB, BenderPoints[n], r_membrane+0.2, 'NodeRing-'+str(n+1))
    Apply_Displacement_BC_Bend(myString, BenderPoints[n], 'BendBC-'+str(n+1), 'Bend', 'Expand', 'Smooth-Bend', d[0], d[1], d[2], rot[0], rot[1], rot[2])
    Rigid_Ring_Constraint(myString, BenderPoints[n], myB+str('-1'), 'NodeRing-'+str(n+1),'RingConstraint'+str(n+1))

##Create Expansion BCs

for n, CSYS_i in zip(range(0,len(Cylind_CSYS_Indices)), Cylind_CSYS_Indices):
    Create_Set_Node_Ring(myString, myDE, OPoints[n], r_expander+0.3, 'NodeRing-'+str(n+1))
    if n == 0 or n == len(Cylind_CSYS_Indices)-1:
        Apply_Displacement_BC_Expand(myString, myDE+str('-1'), 'NodeRing-'+str(n+1), 'ExpandBC-'+str(n+1),'Smooth-Expand', 'Expand', CSYS_i, 0.0, 0.0, 0.0, UNSET, UNSET, UNSET)
    elif OPoints[n][2] > 0:
        Apply_Displacement_BC_Expand(myString, myDE+str('-1'), 'NodeRing-'+str(n+1), 'ExpandBC-'+str(n+1),'Smooth-Expand', 'Expand', CSYS_i, expander_dispSVC, 0.0, 0.0, UNSET, UNSET, UNSET)
    else:
        Apply_Displacement_BC_Expand(myString, myDE+str('-1'), 'NodeRing-'+str(n+1), 'ExpandBC-'+str(n+1),'Smooth-Expand', 'Expand', CSYS_i, expander_dispIVC, 0.0, 0.0, UNSET, UNSET, UNSET)
...
for n, CSYS_i in zip(range(0,len(Cylind_CSYS_Indices)), Cylind_CSYS_Indices):
    if n == 0 or n == len(Cylind_CSYS_Indices)-1:
        Apply_Displacement_BC_Expand(myString, myDE+str('-1'), 'NodeRing-'+str(n+1), 'RecoilBC-'+str(n+1),'Balloon-Recoil', 'Recoil', CSYS_i, 0.0, 0.0, 0.0, UNSET, UNSET, UNSET)
    elif OPoints[n][2] > 0:
        Apply_Displacement_BC_Expand(myString, myDE+str('-1'), 'NodeRing-'+str(n+1), 'RecoilBC-'+str(n+1),'Balloon-Recoil', 'Recoil', CSYS_i, expander_dispSVC, 0.0, 0.0, UNSET, UNSET, UNSET)
    else:
        Apply_Displacement_BC_Expand(myString, myDE+str('-1'), 'NodeRing-'+str(n+1), 'RecoilBC-'+str(n+1),'Balloon-Recoil', 'Recoil', CSYS_i, expander_dispIVC, 0.0, 0.0, UNSET, UNSET, UNSET)


## Pin Stent in Recoil 
Create_Set_Nodes(myString, myS, [[-3.64658,-1.510496,36.5],[3.646625,-1.510388,36.499985],[3.64662,1.5104,36.499985]], 'Recoil-Pins')
Apply_Displacement_BC_Pin(myString, myS+str('-1'), 'Recoil-Pins', pin_i, 'Recoil-Pins', 'Recoil', UNSET, UNSET, UNSET, 0, UNSET, UNSET, UNSET)
...
Set_SFM3D3_Elem_Type(myString, myDE, 'ALL-Elements')
...
Create_Surface_Orphan_Mesh(myString, myDE, 'internal')
Create_Surface_Orphan_Mesh(myString, myDE, 'external')
...
Create_Interaction(myString, 'Bend', 'Contact-Bending', [myDE+str('-1'), myV+str('-1')], 'Int-Prop-1', 'Expand')
Create_Interaction(myString, 'Expand', 'Contact-Expansion', [myB+str('-1')], 'Int-Prop-1', 'None')

## Mass Scaling 
Mass_Scale(myString, 'Bend', 1000)
